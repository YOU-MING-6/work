---
# 文章标题
title: 作品文档
# 自定义封面图片
cover:
# 页面图标
icon: book
# 侧边栏顺序
order: 8
# 设置作者
author:
# 设置写作时间
date: 2024-07-27
# 分类
category:
  - 更新历史
# 标签
tag:
  - 更新历史
# 此页面会在文章列表置顶
sticky: false
# 此页面会出现在星标文章中
star: false
# 自定义页脚
footer:
# 自定义版权信息
copyright:
---

::: info 这是哪？
这是作品文档的更新历史文档\
[返回主文档](/作品文档/作品文档.html)
:::

---

## <HopeIcon icon="circle-info" /> 基本信息

::: tip 更新状态
此作品目前 `正在更新`\
当前为 `正式版`
:::

::: info 项目
这是一个 `YM 网站项目`\
这是一个开源项目。
:::

---

::: info 创作人员
参与创作的人员：`@柚明`
::: 

## <HopeIcon icon="clock-rotate-left" /> 更新历史

### 26日 <Badge text="正式版" type="tip" vertical="middle" />
- 🔥重磅更新

  - 修了4天 cloudflare 部署失败的问题 ，最终放弃
  - 使用 GitHub-页面 部署，成功，并正式投入使用

::: right
**2024.7.26**
:::

---

### 23日 <Badge text="内测版" type="note" vertical="middle" />
- ⭐更新

  - 使用 git 命令上传源文件至 GitHub
  - 使用 cloudflare 部署

::: right
**2024.7.23**
:::

---

### 22日 <Badge text="内测版" type="note" vertical="middle" />
- ⭐更新

  - 修改了很多内容，进一步自定义页面
  - 优化了作品文档

::: right
**2024.7.22**
:::

---

### 21日 <Badge text="内测版" type="note" vertical="middle" />
- ⭐更新

  - 删除原文档内容，使用作品文档排版
  - 将 飞书文档 内容搬迁至此
  - 修改导航栏、侧边栏、底部信息
  - 修改了首页样式

::: right
**2024.7.21**
:::

---

### 2.0.0 <Badge text="内测版" type="note" vertical="middle" />
- ⭐更新

  - 弃用 VuePress ，使用 VuePress Theme Hope 进行开发
  - 固定版本号

::: right
**2024.7.21**
:::

---

### 1.0.0 <Badge text="内测版" type="note" vertical="middle" />
- ⭐更新

  - 使用 VuePress 进行开发

::: right
**2024.7.19**
:::

---

::: tip 开始创作
- 由 `YOU MING 工作室` 的用户 `@柚明` 想到灵感，而后开始创作

::: right
**2024.7.19**
:::

---

::: info 返回
[返回主文档](/作品文档/模板.html)
:::